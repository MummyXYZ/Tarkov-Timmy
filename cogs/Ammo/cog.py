...
class Ammo(commands.Cog):
    ...
    sort = [Choice(name="Penetration", value="pen"), Choice(name="Damage", value="damage"), Choice(name="Fragmentation Chance", value="frag"), Choice(name="Recoil", value="recoil")]
    ...
    caliber: str, sort: str = "pen", hidden: bool = True):
        ...
        body, ammoList = [], []
        ammos = json.loads(json.dumps([ammo for ammo in ammoJson if ammo['Caliber'] == caliber]))
        
        for ammo in ammos:

            ammoName:str = (' '.join(ammo['Name'].split(" ")[1:]))\
                .replace("Blackout", "")\
                .replace(" slug", "")\
                .replace(" buckshot", "")\
                .replace(" Premier HP", "")\
...
# .upper()
        ...
        for ammo in ammoSorted:
            body.append([ammo['name'], ammo['pen'], ammo['damage'], f"{ammo['frag']}%", f"{ammo['recoil']}"])
        
        header = ["", "PEN", "DMG", "FRAG%", "RECOIL"]
        t2ascii = table2ascii(header=header, body=body, style=PresetStyle.thin_double_rounded)
