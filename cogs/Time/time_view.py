...
class TimeView(discord.ui.View):
    def __init__(self):
        def inGameTime(left) -> str:
            ...
            tarkyMultiplier = 7
            tarkyTime = datetime.fromtimestamp((((utc_Time * tarkyMultiplier) % 86400000))) + russiaOffset + leftOffset
            tarkyTimeF = tarkyTime.strftime("%H:%M:%S")
            return tarkyTimeF

        super().__init__(timeout=None)
        self.add_item(TimeButton(custom_id=f"{VIEW_NAME}:time_button", label=f"{inGameTime(True)} ⌚ {inGameTime(False)}"))
