...
class Map(commands.Cog):
    ...
    for location in conf['locations']:
        ...
        for y in mapsJson:
            if y['Map Internal Name'] == loc_Data['internal'] and y['Map Locked'] == False:
                maps.append(Choice(name=location.title(), value=location))
                

    @app_commands.command(name = "map", description = "Maps and information for a specific location")
    @app_commands.describe(map="Specify the map", hidden="Hide message?")
    @app_commands.choices(map=maps)
    async def map(self, interaction: discord.Interaction,
    map: str, hidden: bool = True):
        ...
        url = 'https://escapefromtarkov.fandom.com/wiki/{0}'.format(map.title().replace(" ", "_").replace("Of", "of"))
        ...
        for mapData in mapsJson:
            if mapData['Map Internal Name'] == conf['locations'][map]['internal']:
                ...
                players = f"{int(mapData['Min Players'])} - {int(mapData['Max Players'])}"

        flavor = f"{mapWiki.find(id='Description').find_next('p').text}"
        image = f"{conf['locations'][map]['base']}/revision/latest/scale-to-width-down/800"
        description = ""
        ...
        for boss in bossJson:
            if boss['MAP_ID'] == conf['locations'][map]['internal']:
                ...
                if boss['BossName'] == 'bossKnight': 
                    goons = True
                    continue
                if boss['BossName'] == 'bossTagilla' and boss['MAP_ID'] == 'factory4_night': continue
                if boss['BossName'] == 'pmcBot' or boss['BossName'] == 'exUsec' or boss['BossName'] == 'gifter' or boss['BossName'] == 'bossBoarSniper': continue
                
                nameTemp = boss['BossName'].removeprefix('boss')
                if nameTemp == "Bully": nameTemp = "Reshala"
                if nameTemp == "Kojaniy": nameTemp = "Shturman"
                if nameTemp == "Boar": nameTemp = "Kaban"
                if bossName == None:
        ...
        await interaction.followup.send(embed = embed, view = MapView(map, conf), ephemeral=True)
