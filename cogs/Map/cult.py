...
class CultButton(discord.ui.Button):
    def __init__(self, origMap, map, conf):
        ...
        self.map = map
        self.conf = conf
        super().__init__(style= discord.ButtonStyle.danger, label="Cultists", custom_id="cult", row=1)
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send(embed=embed, view=CultView(self.conf), ephemeral=True)
...
class CultView(discord.ui.View):
    def __init__(self, conf):
        ...
        for map in conf['cultists']['maps']:
            self.add_item(CultSubButton(conf, label=f"{map.capitalize()} Map", custom_id=map))
