...
class MapView(discord.ui.View):
    def __init__(self, origMap, conf):
        ...
        for boss in bossJson:
            if boss['Map'] == origMap or (boss['Map'] == "Sandbox" and origMap == "Ground Zero"):
        ...
        if "interactive" in conf['locations'][origMap]: self.add_item(Button(label="Interactive Map", url=conf['locations'][origMap]['interactive']))

        for maps in conf['locations'][origMap]:
            if maps != "duration" and maps != "players" and maps != "base" and maps != "interactive" and maps != "internal":
...
class MapButton(discord.ui.Button):
    def __init__(self, origMap, map, conf):
        ...
        self.map = map
        self.conf = conf
        super().__init__(style= discord.ButtonStyle.primary, label="2D Map" if map == "map" else map.capitalize(), custom_id=map)

    async def callback(self, interaction: discord.Interaction):
        author = "2D Map" if self.custom_id == "map" else f"{self.custom_id.capitalize()} Map"
