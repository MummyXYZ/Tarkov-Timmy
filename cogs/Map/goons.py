...
class GoonsButton(discord.ui.Button):
    def __init__(self, origMap, map, conf):
        ...
        self.map = map
        self.conf = conf
        super().__init__(style= discord.ButtonStyle.danger, label="The Goons", custom_id="goons", row=1)
...
class GoonsView(discord.ui.View):
    def __init__(self, conf):
        ...
        for map in conf['thegoons']['maps']:
            self.add_item(GoonsSubButton(conf, label=f"{map.capitalize()} Map", custom_id=map))
